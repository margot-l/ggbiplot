...
#' @param colour.group          optional factor variable indicating the colour.group that the observations belong to. If provided the points will be colored according to colour.group
#' @param shape.group          optional factor variable indicating the shape.group that the observations belong to. If provided the points will be shaped according to shape.group
...
ggbiplot <- function(pcobj, choices = 1:2, scale = 1, pc.biplot = TRUE, 
                     ...
                     colour.group = NULL, shape.group=NULL, ellipse = FALSE, ellipse.prob = 0.68, 
                     ...
                     varname.abbrev = FALSE, ...)
{
  ...
  if(!is.null(df.u$labels)) {
    ...
    if(!is.null(df.u$colour.group)&!is.null(df.u$shape.group)) {
      g <- g + geom_point(aes(color = colour.group, shape = shape.group), 
                         alpha = alpha)
    }else if(!is.null(df.u$shape.group)) {
      g <- g + geom_point(aes(shape = shape.group), alpha = alpha)
    }else if(!is.null(df.u$colour.group)) {
  ...
  # Overlay a concentration ellipse if there are colour.group
  ...
  # Change the name of the legend for colour.group
  # if(!is.null(colour.group)) {
  #   g <- g + scale_color_brewer(name = deparse(substitute(colour.group)), 
  #                               palette = 'Dark2')
  # }
